...
generate_genomic_tracks <- function(gr_list, locus, window, max_score, colors) {
  ...
  displayPars(track) <- list(shape = "box", stackHeight = 0.5)

  return(track)
}


plot_tracks <- function(TxDb, gene2symbol, plot_list, locus, window) {
  locus <- gene_coord[gene_coord$name %in% locus, ]
  ...
  plot <- plotTracks(
    c(plot_list, gene_track, genome_track),
    chromosome = as.character(seqnames(locus_gr)),
    from = start(locus_gr),
    to = end(locus_gr),
    ...
    alpha.title = 1,
    alpha.axis = 1,
    sizes = c(rep(0.4, length(plot_list)), 0.4, 0.3),
    cex.title = 0.9,
    cex.id = 0.5,
    cex.axis = 0.7,
    title.width = 1.4
...
plot <- foreach(gene = genes) %dopar% {
  ...
  png(file = paste0(save_folder, "/", gene, "_RNA_tracks.png"), width = 6.5, height = 3.7, units = "in", res = 150)
  # png(file=paste0(gene,"_RNA_tracks.png"), width=7.5, height=5, units = 'in', res=150)
